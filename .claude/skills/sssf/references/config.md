...
| `kiro_cli` | **no** | **no** — bills credits | yes, exact: summed from its own context breakdown |
| `antigravity` | yes | no — bills AI credits | yes (last turn's total), window unknown |

Kiro CLI reports neither billed tokens nor dollars: it bills **credits**, per turn and explicitly unit-labelled (`_meta.kiro.promptTurnSummaries[] = {"unit": "credit", "usage": 0.0275}`). `usage.total_tokens` and `usage.total_cost` therefore stay 0 for a `kiro_cli` agent, so a roster's `sessions.total_tokens` will undercount if it mixes Kiro with other harnesses. The credits figure is not silently dropped — it stays verbatim in `raw_output.jsonl` — but there is no field for it on `UsageBreakdown`, whose cost columns are dollars for every other producer.
